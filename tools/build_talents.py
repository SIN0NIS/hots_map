# -*- coding: utf-8 -*-
"""js/data_talents.js 를 만든다 — 저장소 관리자용.

리플레이의 TalentChosen 이벤트는 특성을 «FenixMobileOffense» 같은 내부 이름으로
남긴다. 화면에 한국어로 보여주려면 게임 데이터의 특성 표가 필요하다.

    python tools/build_talents.py [herodata_kokr.json 경로]

기본 경로는 이 저장소 밖의 백과사전 파이프라인 산출물이다. 없으면 건너뛴다
(그 경우 뷰어는 내부 이름을 그대로 보여준다).
"""
import json
import os
import re
...
DEFAULT_SRC = os.path.join(
    os.path.dirname(os.path.dirname(ROOT)), "03_claude",
    "hots_github_date260725", "06_auto_encyclopedia", "work",
    "herodata_97650_kokr.json")
...
def main():
    ...
    if not os.path.isfile(src):
        raise SystemExit(f"영웅 데이터가 없다: {src}")
        ...
        for level, arr in (hero.get("talents") or {}).items():
            ...
            for t in arr:
                ...
                for k in ("nameId", "buttonId"):
                    if t.get(k):
                        tab.setdefault(norm(t[k]), entry)
                for a in (t.get("abilityTalentLinkIds") or []):
                    if a and ":" not in a:
                        tab.setdefault(norm(a) + norm(t.get("name", "")), entry)
    ...
    # 실제로 리플레이에 나오는 이름으로 바로 찾을 수 있게 «내부명 -> 한국어» 표를 굽는다.
    talents = {}
    for hero in data.values():
        for level, arr in (hero.get("talents") or {}).items():
            lv = int(level.replace("level", ""))
            for t in arr:
                for k in ("nameId", "buttonId"):
                    if t.get(k):
                        talents.setdefault(t[k], {"ko": t.get("name", ""), "lv": lv})
    # 표에 없는 변형 이름도 미리 풀어 둔다 (샘플 리플레이에 실제로 나온 것들)
    ...
    with open(out, "w", encoding="utf-8", newline="\n") as fp:
        fp.write("// 자동 생성 — tools/build_talents.py. "
                 "특성 내부명 -> {ko: 한국어명, lv: 티어 레벨}\n"
                 "// 출처: 게임 데이터(herodata ko-KR). "
                 "리플레이의 TalentChosen.PurchaseName 이 이 키다.\n"
                 "const TALENT_DB = "
